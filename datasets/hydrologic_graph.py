"""Hydrologic dataset views and event-balanced same-graph batching.

The dataset preserves the frozen event/split/sample-origin facts but changes two runtime
properties only:
...
from datasets.hydrologic_graph_base import (
    HydrologicGraphBaseDataset,
    _norm_id,
)
...
class HydrologicGraphDataset(HydrologicGraphBaseDataset):
    """Load hydrologic tensors without mutating the underlying on-disk dataset."""
    ...
    def __init__(
        ...
        *,
        ...
        # Use a dedicated constructor: the 72 h dataset has an explicit contract.
        ...
        if self.contract.get("contract") != CONTRACT_NAME:
            raise ValueError(
                f"not the supported 72h-rain contract: {self.contract.get('contract')!r}"
            )
        ...
        if (
            ...
            raise ValueError(
                "fixed design requiresrain_history=72 h、Q/Z history=24 h、forecast=6 h"
            )
        ...
        if missing:
            raise ValueError(f"dataset sample_index缺少字段: {sorted(missing)}")
        ...
        if samples.empty:
            raise ValueError(f"{self.split}has no usable hydrologic samples")
        ...
        if invalid_phase.any():
            ...
            raise ValueError(f"dataset EVENT_PHASE非法: {values}")
        ...
        self.train_sampling_mode = (
            "event_balanced_phase_stratified"
            if self.split == "TRAIN" and self.require_q_supervision
            else "full_view_same_graph_batches"
        )
...
def validate_hydrologic_batch(batch: HydrologicGraphBatch) -> None:
    ...
    for name in (
        ...
        if not torch.isfinite(getattr(batch, name)).all():
            raise ValueError(f"hydrologic {name}含NaN/Inf")
    if (batch.history_rain < 0).any() or (batch.future_rain < 0).any():
        raise ValueError("rainfall forcing必须非负")
    if (batch.incremental_area_km2 <= 0).any():
        raise ValueError("incremental_area_km2必须>0")
    topological_levels(batch.edge_index, n)


def collate_hydrologic_graph(
    ...
    if any(not isinstance(value, str) for value in graph_ids) or len(set(graph_ids)) != 1:
        raise ValueError(f"一个batch只能来自同一GRAPH_ID，实际={graph_ids}")
    static_fields = (
        "node_static",
        "incremental_area_km2",
        "edge_index",
    ...
    if any(item.obs_station_ids != items[0].obs_station_ids for item in items[1:]):
        raise ValueError("同图batch内obs_station_ids不一致")
    ...
    for name in (
        ...
        if any(value is None for value in values):
            raise ValueError(f"batch内{name}不能为None")
...
class EventBalancedBatchSampler(Sampler[list[int]]):
    ...
    def __init__(
        self,
        dataset: HydrologicGraphDataset,
        batch_size: int,
        *,
        ...
        if dataset.split != "TRAIN" or not dataset.require_q_supervision:
            raise ValueError("event-balanced sampler只允许 Q-supervised TRAIN")
        ...
        if phase_quota * len(EVENT_PHASES) != origins_per_event:
            raise ValueError("fixed design固定四phase等额配额，phase_quota*4必须等于origins_per_event")
        ...
        if not by_event:
            raise ValueError("TRAIN没有event-balanced候选")
        ...
        if self._length <= 0:
            raise ValueError("event-balanced sampler没有batch")
    ...
    def _select_event(self, event: str, rng: random.Random) -> list[int]:
        ...
        if len(selected) != target or len(set(selected)) != len(selected):
            raise RuntimeError(f"{event}: event sampling计数/去重失败")
        return selected
...
def build_hydrologic_graph_loader(
    dataset: HydrologicGraphDataset,
    *,
    batch_size: int,
    shuffle: bool,
    ...
    if num_workers != 0:
        raise ValueError("current NPZ cache固定num_workers=0")
    if event_balanced_train:
        if not shuffle:
            raise ValueError("event-balanced TRAIN要求shuffle=true")
        batch_sampler: Sampler[list[int]] = EventBalancedBatchSampler(
            dataset,
            batch_size,
        ...
        # Reuse the proven same-graph grouping logic but swap in the hydrologic collate.
        from datasets.hydrologic_graph_base import GraphGroupedBatchSampler

        batch_sampler = GraphGroupedBatchSampler(
            dataset,
            batch_size,
            shuffle=shuffle,
            drop_last=False,
    ...
    return DataLoader(
        ...
        collate_fn=collate_hydrologic_graph,
    )
