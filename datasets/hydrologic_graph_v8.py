"""v8 NPZ-backed loader for hydrologic computational graphs and sparse Q/Z observations."""
...
class HydrologicGraphV8Dataset(Dataset[HydrologicGraphBatch]):
    """Materialise only frozen v8 sample rows, keeping Nnode and Nobs separate."""
    ...
    def __init__(
        ...
        *,
        ...
        if (self.history_hours, self.forecast_hours) != (24, 6):
            raise ValueError("v8固定要求history=24、forecast=6")
        ...
        if missing:
            raise ValueError(f"v8 sample_index缺少字段: {sorted(missing)}")
        ...
        if samples.empty:
            raise ValueError(f"{self.split}没有可用v8样本")
        ...
        self.train_sampling_mode = "v8_full_pass_same_graph_batches"
    ...
    def _load_tensor_file(self, relative_name: str) -> dict[str, np.ndarray]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"v8 tensor不存在: {path}")
        ...
        if set(arrays) != expected:
            ...
            raise ValueError(
                f"{path.name}: NPZ key与v8契约不一致，missing={sorted(missing)}"
            )
...
def collate_hydrologic_graph_v8(
    ...
    for name in static_fields:
        ...
        if any(not torch.equal(first, getattr(item, name)) for item in items[1:]):
            raise ValueError(f"同图v8 batch内{name}不一致")
    if any(item.obs_station_ids != items[0].obs_station_ids for item in items[1:]):
        raise ValueError("同图v8 batch内obs_station_ids不一致")
    ...
    for name in stack_fields:
        ...
        if any(value is None for value in values):
            raise ValueError(f"v8 batch内{name}不能为None")
    ...
    validate_v8_batch(batch)
    return batch


class GraphGroupedV8BatchSampler(Sampler[list[int]]):
...
def build_hydrologic_graph_v8_loader(
    dataset: HydrologicGraphV8Dataset,
    *,
    batch_size: int,
    shuffle: bool,
    ...
    sampler = GraphGroupedV8BatchSampler(
        dataset,
        batch_size,
        shuffle,
        drop_last=False,
    ...
    return DataLoader(
        ...
        collate_fn=collate_hydrologic_graph_v8,
