"""Continuous-time two-reservoir rainfall-runoff module for v9.
    ...
    def forward(
        ...
        with torch.autocast(device_type=features.device.type, enabled=False):
            ...
            if x.ndim != 4 or p.ndim != 3:
                raise ValueError("v9 runoff要求features=[B,T,N,D], rain=[B,T,N,1]")
            b, t, n, _ = x.shape
            ...
            if initial_state is None:
                ...
                if h0.shape != (b, n, self.hidden_dim) or c0.shape != h0.shape:
                    raise ValueError("v9 runoff initial h/c必须为[B,N,hidden_dim]")
                if sf0.shape != (b, n) or ss0.shape != (b, n):
                    raise ValueError("v9 runoff initial storage必须为[B,N]")
                if (
                    ...
                    raise ValueError("v9 runoff initial state非法")
                state = (
