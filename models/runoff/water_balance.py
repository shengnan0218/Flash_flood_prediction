...
class WaterBalanceLSTMCell(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int) -> None:
        ...
        self.controls = nn.Linear(hidden_dim, 3)
...
class WaterBalanceLSTM(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int) -> None:
        ...
        self.cell = WaterBalanceLSTMCell(input_dim, hidden_dim)
    ...
    def forward(
        ...
        with torch.autocast(device_type=features.device.type, enabled=False):
            ...
            if initial_state is None:
                ...
                if h0.shape != (b, n, self.hidden_dim) or c0.shape != (
                    b,
                    n,
                    self.hidden_dim,
                ):
            ...
            stores: list[torch.Tensor] = []
            fast_stores: list[torch.Tensor] = []
            slow_stores: list[torch.Tensor] = []
            k_fast: list[torch.Tensor] = []
            k_slow: list[torch.Tensor] = []
            ...
            for i in range(t):
                ...
                qs.append(r.reshape(b, n) * area[None] * 1000.0 / seconds)
                residual.append(d["residual"].reshape(b, n))
                fast_stores.append(d["storage_fast"].reshape(b, n))
                slow_stores.append(d["storage_slow"].reshape(b, n))
                stores.append(
                    (d["storage_fast"] + d["storage_slow"]).reshape(b, n)
                )
                k_fast.append(d["k_fast"].reshape(b, n))
                k_slow.append(d["k_slow"].reshape(b, n))
                partition.append(d["g"].reshape(b, n))
            return torch.stack(qs, 1), {
                "runoff_water_balance_residual": torch.stack(residual, 1),
                "storage": torch.stack(stores, 1),
                "storage_fast": torch.stack(fast_stores, 1),
                "storage_slow": torch.stack(slow_stores, 1),
                "k_fast": torch.stack(k_fast, 1),
                "k_slow": torch.stack(k_slow, 1),
                "rain_partition_fast": torch.stack(partition, 1),
            }
...
class PureLSTMRunoff(nn.Module):
    ...
    def forward(
        ...
        y, _ = self.lstm(
            features.permute(0, 2, 1, 3).reshape(b * n, t, d)
        )
        q = self.head(y).reshape(b, n, t).permute(0, 2, 1)
        return q, {
