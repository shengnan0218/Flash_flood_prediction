"""v9 Q/Delta-Z objective with independent masks and no global Q-Z consistency."""
...
_COMPONENTS = ("q_point", "q_peak", "q_volume", "z_level", "z_slope")


class HydrologicGraphV9Loss:
    """Q point/peak/volume + Delta-Z level/slope; Q and Z remain independent tasks."""
    ...
    def __init__(self, cfg: Mapping[str, Any]) -> None:
        ...
        if changed:
            raise ValueError(f"v9正式loss权重与冻结设计不一致: {changed}")
        runtime = cfg.get("_runtime", {})
        normal = runtime.get("v8_normalization")
        if not isinstance(normal, Mapping):
            raise ValueError("v9 loss缺少TRAIN-only normalization")
        ...
        if (
            ...
            raise ValueError("v9 per-station Q/Delta-Z scale非法")
    ...
    def batch_statistics(
        ...
        peak_prediction = torch.where(q_active, peak_prediction, torch.zeros_like(peak_prediction))
