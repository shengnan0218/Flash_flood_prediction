"""Formal V11 Q-only objective with TRAIN-only high-flow emphasis.

V11 deliberately removes the V10 sliding-window maximum loss.  High-flow
...
class HydrologicGraphV11Loss:
    def __init__(self, cfg: Mapping[str, Any]) -> None:
        self.cfg = cfg
        loss = cfg.get("loss", {})
        if str(loss.get("mode", "")).lower() != "q_only_high_flow":
            raise ValueError("v11正式loss.mode必须为q_only_high_flow")
        expected = {
            "q_point_weight": 1.0,
        ...
        if changed:
            raise ValueError(f"v11 Q loss设计不一致: {changed}")
        if "q_peak_weight" in loss:
            raise ValueError("v11禁止保留sliding-window q_peak_weight")

        runtime = cfg.get("_runtime", {})
        normal = runtime.get("v8_normalization")
        if not isinstance(normal, Mapping):
            raise ValueError("v11 loss缺少TRAIN-only Q normalization")
        ...
        if (
            ...
            raise ValueError("v11 per-station Q scale非法")

        station_ids = tuple(str(value) for value in runtime.get("v8_station_ids", ()))
        quantiles = runtime.get("v11_high_flow_quantiles")
        if not station_ids or not isinstance(quantiles, Mapping):
            raise ValueError("v11 loss缺少station catalogue或TRAIN-only high-flow quantiles")
        station_stats = quantiles.get("stations")
        if not isinstance(station_stats, Mapping):
            raise ValueError("v11 high-flow quantiles缺少stations")
        ...
        for index, station in enumerate(station_ids):
            ...
            if not math.isfinite(q80) or not math.isfinite(q99) or q99 <= q80:
                raise ValueError(f"v11 high-flow threshold非法: {station}: {q80}, {q99}")
            lower[index] = q80
            upper[index] = q99
            available[index] = True
        if len(station_ids) != int(self.q_scales.numel()):
            raise ValueError("v11 Q scale与station catalogue长度不一致")
    ...
    def _checked(
        ...
        if prediction.shape != target.shape or mask.shape != target.shape:
            raise ValueError("v11 Q prediction/target/mask shape必须一致")
        valid = mask.bool()
        if valid.any():
            if not torch.isfinite(prediction[valid]).all():
                raise FloatingPointError("v11有效Q预测含NaN/Inf")
            if not torch.isfinite(target[valid]).all():
                raise ValueError("v11有效Q target含NaN/Inf")
        return valid
