"""Station/basin/event/lead-time evaluation.

Q is reported in m3/s. Z follows the current dataset target semantics
...
def _batch_strings(value: Any, batch_size: int, label: str) -> tuple[str, ...]:
    ...
    raise ValueError(f"hydrologic evaluation要求{label}包含每个sample的元数据")
...
def _load_station_metadata(dataset: Any):
    ...
    if not path.is_file():
        raise FileNotFoundError(f"hydrologic evaluation缺少station mapping: {path}")
    ...
    *,
    ...
    for batch in loader:
        ...
        if q_pred_t.shape != batch.q_target.shape or z_pred_t.shape != batch.z_target.shape:
            raise ValueError("hydrologic evaluation prediction/target shape不一致")
        batch_size = int(q_pred_t.shape[0])
        if q_pred_t.shape[1] != horizon:
            raise ValueError("hydrologic evaluation forecast horizon不一致")
        ...
        if any(_normalise_id(value) != graph_id for value in graph_ids):
            raise ValueError("hydrologic evaluation batch混入多个graph")
    ...
    if sample_total == 0:
        raise ValueError("hydrologic evaluation loader为空")
