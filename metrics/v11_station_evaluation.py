...
def _unix_hour(value: str) -> int:
    ...
    if not math.isfinite(float(offset)) or not math.isclose(
        ...
        raise ValueError(f"v11 evaluation FORECAST_TIME不是整小时: {value}")
    return int(rounded)
    ...
    *,
    ...
    for batch in loader:
        ...
        if q_true.shape != q_pred.shape or q_mask.shape != q_pred.shape:
            raise ValueError("v11 evaluation Q prediction/target/mask shape不一致")
        ...
        if not duplicate.empty:
            for _, part in duplicate.groupby("TARGET_UNIX_HOUR", sort=False):
                if (
                    ...
                    raise ValueError(
                        "v11 fixed-lead event series存在冲突duplicate"
                    )
