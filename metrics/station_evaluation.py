"""Station, event and graph evaluation for Q-only prediction and derived stage."""
...
from metrics.station_evaluation_core import (
    _array_hydrograph,
    _array_sums,
    _batch_strings,
    _empty_hydrograph,
    ...
    *,
    ...
    for batch in loader:
        ...
        if len(outlet_matches) != 1:
            raise ValueError(
                f"{graph_id}: hydrologic evaluation必须唯一定位outlet={outlet_station}，"
    ...
    for graph_id, event_id, station_id in sorted(event_q):
        ...
        if meta is None:
            raise ValueError(
                f"event evaluation缺station metadata: {graph_id}/{station_id}"
            )
