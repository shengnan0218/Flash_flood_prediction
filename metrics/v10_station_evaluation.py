"""Final station-aware evaluation for v10 Q-only + rating-derived stage."""
...
def evaluate_v10_station_aware(
    ...
    *,
    ...
    """Evaluate Q plus explicitly masked, non-neural stage products in one pass."""
    model = trainer.model
    ...
    for batch in loader:
        ...
        raw_available = output["z_rating_raw_available_mask"].detach().cpu().numpy().astype(bool)
        ...
        _merge(graph_z_abs[graph_id], _array_sums(z_abs_pred, z_abs_true, corrected_mask))

        for lead in range(horizon):
            _merge(lead_q[lead], _array_sums(q_pred[:, lead], q_true[:, lead], q_mask[:, lead]))
            _merge(
                lead_dz[lead],
                _array_sums(dz_pred[:, lead], dz_true[:, lead], corrected_mask[:, lead]),
            )
            _merge(
                lead_z_abs[lead],
                _array_sums(z_abs_pred[:, lead], z_abs_true[:, lead], corrected_mask[:, lead]),
            )

        for obs, station in enumerate(batch.obs_station_ids):
            station_id = _normalise_id(station)
            _merge(
            ...
            _merge(
                ...
                _array_sums(
                    dz_pred[:, :, obs], dz_true[:, :, obs], corrected_mask[:, :, obs]
                ),
            )
            _merge(
                station_z_abs[station_id],
                _array_sums(
                    z_abs_pred[:, :, obs], z_abs_true[:, :, obs], corrected_mask[:, :, obs]
                ),
            )
            _merge(
    ...
    for (graph_id, station_id), meta in sorted(station_meta.items()):
        ...
        row = {
            ...
            **_regression_fields("delta_z_rating_corrected", station_dz[station_id]),
            **_regression_fields("z_abs_rating_corrected", station_z_abs[station_id]),
            **_regression_fields("z_abs_rating_unanchored", station_z_raw[station_id]),
        }
        ...
        graph_rows.append(
            {
                ...
                **_regression_fields("q", graph_q[graph_id]),
                **_regression_fields("delta_z_rating_corrected", graph_dz[graph_id]),
                **_regression_fields("z_abs_rating_corrected", graph_z_abs[graph_id]),
            }
        )

    step_seconds = float(trainer.cfg.get("temporal", {}).get("target_step_seconds", 3600.0))
        ...
        lead_rows.append(
            {
                ...
                **_regression_fields("q", lead_q[lead]),
                **_regression_fields("delta_z_rating_corrected", lead_dz[lead]),
                **_regression_fields("z_abs_rating_corrected", lead_z_abs[lead]),
            }
        )
    ...
    summary = {
        ...
        "stage_prediction_method": "Q-derived TRAIN-only station linear rating + forecast-origin Z residual correction",
        ...
        "derived_stage": {
            ...
            **_regression_fields("z_abs_rating_unanchored", z_raw_global),
            ...
            "forecast_origin_q_observed_count": q0_observed_total,
            "forecast_origin_z_observed_count": z0_observed_total,
        },
        "rating_curve_audit": trainer.cfg.get("_runtime", {}).get("v10_rating_curves", {}),
    }
    payload = {
        "summary": summary,
        "files": {
