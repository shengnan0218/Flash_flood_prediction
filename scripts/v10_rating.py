"""TRAIN-only station rating calibration for formal v10.

The frozen v8 data remain the source of truth.  Curves are fitted from unique
...
NOT required for fitting; it is only an operational anchor used by the v10 model.
"""
...
def _fit_ols(q: np.ndarray, z: np.ndarray) -> dict[str, float]:
    ...
    if q.ndim != 1 or z.shape != q.shape or q.size < 2:
        raise ValueError("v10 rating OLS要求至少2个一维Q/Z配对点")
    q_mean = float(q.mean())
    z_mean = float(z.mean())
    denominator = float(np.square(q - q_mean).sum())
    if not math.isfinite(denominator) or denominator <= 0:
        raise ValueError("v10 rating TRAIN Q无方差")
    ...
    if not math.isfinite(slope) or not math.isfinite(intercept) or slope <= 0:
        raise ValueError("v10 rating要求TRAIN-only线性Q-Z斜率为有限正数")
...
def fit_train_only_linear_ratings(
    ...
    *,
    ...
    if not sample_path.is_file() or not mapping_path.is_file():
        raise FileNotFoundError("v10 rating需要v8 sample_index和station mapping")
    ...
    if missing:
        raise ValueError(f"v10 rating sample_index缺字段: {sorted(missing)}")
    ...
    for relative_name, group in samples.groupby("TENSOR_FILE", sort=True):
        ...
        if root not in path.parents or not path.is_file():
            raise ValueError(f"v10 rating非法TENSOR_FILE: {relative}")
                ...
                for obs, station in enumerate(local_stations):
                    for lead in range(horizon):
                        ...
                        if not z0_mask[obs] or not np.isfinite(z0[obs]):
                            raise ValueError("v10 rating发现Delta-Z有效但Z0无效")
                        ...
                        if not math.isfinite(q_value) or not math.isfinite(z_value):
                            raise ValueError("v10 rating有效Q/Z含NaN/Inf")
    ...
    if duplicate_conflicts:
        raise ValueError(f"v10 rating重叠TRAIN窗口存在{duplicate_conflicts}个矛盾Q/Z值")
    ...
    for field in ("STATION_ID", "IS_OUTLET_STATION"):
        if field not in mapping.columns:
            raise ValueError(f"v10 rating mapping缺少{field}")
    ...
    if require_all_outlet_stations and missing_outlets:
        raise ValueError(
            "v10正式stage要求所有outlet有TRAIN-only rating curve，缺失="
            f"{missing_outlets}"
        )
