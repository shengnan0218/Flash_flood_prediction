"""Trainer for the single supported Q-only hydrologic model."""
...
class HydrologicTrainer(Trainer):
    """Generic fit/checkpoint mechanics with Q and persistence diagnostics."""

    def __init__(
        self, model: torch.nn.Module, cfg: dict, device: torch.device
    ) -> None:
        # Initialize the shared optimizer directly for the Q-only objective.
        ...
        if self.selection_mode != "val_loss":
            raise ValueError("Q-only training requires val_loss checkpoint selection")
        self.best = float("inf")
        self.start_epoch = 0
        self.stale = 0
        self.last_epoch = -1
    ...
    def evaluate(
        ...
        *,
        ...
        Extra metrics are diagnostics only. Checkpoint selection remains
